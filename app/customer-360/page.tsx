            ...
            <table className="w-full text-left border-collapse min-w-[700px]">
              <thead>
                <tr className="bg-[#111] border-b border-[#222]">
                  <th className="px-4 py-2.5 text-xs font-semibold text-[#888] uppercase tracking-wider">Customer / Entity</th>
                  <th className="px-4 py-2.5 text-xs font-semibold text-[#888] uppercase tracking-wider">Type</th>
                  <th className="px-4 py-2.5 text-xs font-semibold text-[#888] uppercase tracking-wider">Risk Score</th>
                  <th className="px-4 py-2.5 text-xs font-semibold text-[#888] uppercase tracking-wider">Next Action</th>
                  <th className="px-4 py-2.5 text-xs font-semibold text-[#888] uppercase tracking-wider text-right">Actions</th>
                </tr>
              </thead>
                ...
                {mockCustomers.map((customer, i) => (
                  <tr key={customer.customer_id} className="hover:bg-white/[0.02] transition-colors group">
                    <td className="px-4 py-3">
                      <div className="font-semibold text-white text-[13px]">{customer.name}</div>
                      <div className="text-[11px] text-[#777] mt-0.5 font-mono">{customer.customer_id}</div>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-white/5 border border-white/10 text-[#AAA]">
                        {customer.type}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center justify-center px-2 py-0.5 rounded text-[11px] font-bold ${getRiskScoreColor(customer.global_risk_score)}`}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button 
                        ...
                        className="px-2.5 py-1.5 bg-[#37b7ab]/10 border border-[#37b7ab]/20 text-[#37b7ab] text-xs font-semibold rounded hover:bg-[#37b7ab]/20 transition-colors"
                      >
                        View 360
                      </button>
                    </td>
      ...
      >
        {selectedCustomer && (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-5 sidebar-scrollbar">
